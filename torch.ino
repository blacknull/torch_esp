...
uint8_t brightness = 128;//brightnessMap[0];
...
const int PIN_BUTTON = 3; //RX, should be high when bootup
const unsigned long INTERVAL_VALID = 50;
const unsigned long INTERVAL_LONGPRESS = 600;
...
ICACHE_RAM_ATTR void isrButton() {
  ...
  if (ms < buttonState.ms + INTERVAL_VALID)
    return;
...
int buttonActionGet() {
  ...
  if (buttonState.ms != 0 && buttonState.ms2 != 0) {
    unsigned long timeDownUp = buttonState.ms2 - buttonState.ms;
    if (timeDownUp == 0 || timeDownUp > 60 * 1000) // invalid data because unknow reason
      timeDownUp = 100;
      
    if (timeDownUp > INTERVAL_LONGPRESS) {
      //Serial.println("got a long press. down-up:" + String(timeDownUp));            
      action = BUTTON_LONGPRESS;
    }
    else {
      //Serial.println("got a click. down-up:" + String(timeDownUp));
      action = BUTTON_CLICK;
    }

    // clear button state
    memset((void*)&buttonState, 0L, sizeof(BUTTON_STATE));
  }
...
int autoPlayDurationSeconds = 60 * 2;
...
const unsigned long LOOP_INTERVAL   = 30; // ms
const unsigned long MAX_BRIGHTNESS  = 128;
const unsigned long FADING_TIME     = 2000; // ms
unsigned long msLastLongPress = 0;
bool lightOn = true;
void loop() {
  ...
  int action = buttonActionGet();
  if (action == BUTTON_CLICK) {
    const unsigned long INTERVAL_ONOFF = 500;
    unsigned long now = millis();
    if (now < msLastLongPress + INTERVAL_ONOFF) {
      lightOn = !lightOn;
      FastLED.clear(true);
      delay(50);
      FastLED.show();
      ...
      move(1); 
      autoPlayTimout = now + (autoPlayDurationSeconds * 1000);
    }
  }
  else if (action == BUTTON_LONGPRESS) { 
    autoplayEnabled = !autoplayEnabled;

    EEPROM.begin(256);
    EEPROM.write(5, (uint8_t)autoplayEnabled);
    EEPROM.end();

    msLastLongPress = millis();
  ...
  */
  ...
  if (lightOn && autoplayEnabled) {
    unsigned long now = millis();
    if (now > autoPlayTimout) {
      move(1);
      autoPlayTimout = millis() + (autoPlayDurationSeconds * 1000);        
    }
    else if (now + FADING_TIME > autoPlayTimout) {  // to fade out
      uint8_t brgt = MAX_BRIGHTNESS * (autoPlayTimout - now) / FADING_TIME;
      FastLED.setBrightness(brgt);
    }
    else {
      unsigned long last_timeout = autoPlayTimout - (autoPlayDurationSeconds * 1000);
      if (now < FADING_TIME + last_timeout) {
        uint8_t brgt = MAX_BRIGHTNESS * (now - last_timeout) / FADING_TIME;
        FastLED.setBrightness(brgt);        
      }
    }
  }
...
void loadSettings() {
  ...
  // auto shift patton
  autoplayEnabled = EEPROM.read(5) == 0 ? false : true;
